 ...
 */
export async function supervisorIteration({ target, dispatchCycle, now, sleep }) {
...
export async function runOrchestratorSupervisor({
  target, dispatchCycle = defaultDispatchCycle, now = () => Date.now(), sleep = realSleep,
} = {}) {
  ...
  while (!stop) {
    const r = await supervisorIteration({ target, dispatchCycle, now, sleep });
    if (r === 'exit') break;
  }
  process.exit(0);
}
