...
| `POST` | `/api/v1/orchestrator/start` | `application/json` — `{ started, alreadyRunning, supervisorPid }`. Starts the detached supervisor (same as `orchestrator start`); a no-op that reports `alreadyRunning: true` if one is live. |
| `POST` | `/api/v1/reset` | `application/json` — `{ orchestrator, runs }`. **Hard reset:** stops the orchestrator supervisor, then SIGTERMs every active agent run. `runs.killed` / `runs.failed` / `runs.skipped` list the outcome per run. |
| `GET` | `/` | HTML — the bundled dashboard UI |

The dashboard exposes both control-plane actions as header buttons — **start orchestrator** and **kill all** (hard reset). The CLI and the UI drive identical behavior through `api/control.js`.
